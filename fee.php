...
if(isset($_POST['upload'])){
   ...
   $filename = "nfile".$_SESSION['username']."_".$time2."."."xls";
...
if($examType=='ete'){
 ...
 //  $sno =$worksheet->getCellByColumnAndRow(1,1)->getValue();
	...
	$tdate = $worksheet->getCellByColumnAndRow(4, $row)->getFormattedValue();
	//echo "<br>Original date = ".$tdate;
	...
	$date=date_create($unixDateTimestamp);
	$newDate = date("Y-m-d", strtotime($tdate));
	//echo "<br> Date =".$newDate;

	$data_to_store1['trans_date']=$newDate;
	...
	//echo "<br> name = ".$p_name;
	$cell= $worksheet->getCellByColumnAndRow(8, $row)->getFormattedValue();
	$old_date = explode('/', $cell); 
	...
	$data_to_store1['birth_date']=$new_data;//$worksheet->getCellByColumnAndRow(8, $row)->getFormattedValue();
...
header('Location: ete_display.php');
...
<div class="row">
     <div class="col-lg-12">
           <h2 class="page-header">Upload Fee Excel File</h2>
     
	<div class="sturegleft  col-lg-12" >  
		<b> Steps for Uploading Transaction Details</b>
	<ol>
